...
qboolean PM_CheckWater()
{
	...
	if ((cont <= CONTENTS_WATER && cont > CONTENTS_TRANSLUCENT) || (cont >= CONTENTS_FOG && cont <= CONTENTS_FLYFIELD))
	{
		...
		if ((cont <= CONTENTS_WATER && cont > CONTENTS_TRANSLUCENT) || (cont >= CONTENTS_FOG && cont <=
			CONTENTS_FLYFIELD))
		{
			...
			cont = pmove->PM_PointContents(point, NULL);
			if ((cont <= CONTENTS_WATER && cont > CONTENTS_TRANSLUCENT) || cont == CONTENTS_FOG)
				// Flyfields never cover the eyes
...
void PM_CheckFalling()
{
	...
	if (pmove->onground != -1 && !pmove->dead && pmove->flFallVelocity >= 220 && pmove->flFallVelocity <=
