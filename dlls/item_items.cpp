...
class CItemBattery : public CItem
{
	...
	BOOL MyTouch(CBasePlayer* pPlayer) override
	{
		...
		if (pPlayer->TakeArmor(armor) && pPlayer->pev->weapons & (1 << WEAPON_SUIT))
		{
			int pct;
			char szcharge[64];
...
int CItemMedicalKit::MyTouch(CBasePlayer* pPlayer)
{
	...
	{
		pev->health = pPlayer->m_rgItems[ITEM_HEALTHKIT] - MAX_MEDKIT;
		//set the amount of charge left in the kit to be the difference
