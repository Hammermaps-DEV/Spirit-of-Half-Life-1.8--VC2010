...
void CGib::SpawnRandomGibs(entvars_t* pevVictim, int cGibs, int notfirst, const char* szGibModel)
{
	...
	studiohdr_t* pstudiohdr = static_cast<studiohdr_t*>((GET_MODEL_PTR(ENT(pGib->pev))));
	if (! pstudiohdr)
		return;
	...
	for (int cSplat = 0; cSplat < cGibs; cSplat++)
	{
		...
		{
			pGib = GetClassPtr(static_cast<CGib*>(nullptr));
			pGib->Spawn(szGibModel);
		}
...
void CGib::BounceGibTouch(CBaseEntity* pOther)
{
	...
	else
	{
		...
		{
			float volume;
			float zvel = Vfabs(pev->velocity.z);

			volume = 0.8 * Vmin(1.0, ((float)zvel) / 450.0);

			CBreakable::MaterialSoundRandom(edict(), static_cast<Materials>(m_material), volume);
		}
	}
}
