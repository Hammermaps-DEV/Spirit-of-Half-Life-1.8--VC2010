...
TYPEDESCRIPTION CTriggerRelay::m_SaveData[] =
{
	...
	//G-Cont. in garagedoordemo code without this stuff, demo don't work with save\load ;)
};
...
void CTriggerRelay::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	...
	if (pev->spawnflags & SF_FIRE_CAMERA) //For triggering once if player see in camera
	{
		...
		if (pActivator && pActivator->IsPlayer())
		{
			if (static_cast<CBasePlayer*>(pActivator)->viewFlags == 0)
			{
	...
	if (pev->message)
		value = CalcLocus_Number(pActivator, STRING(pev->message)/*LRC 1.8 removed ,pev->skin*/);
	//AJH skin (ratio to return)
...
void CWatcherRatio::Think(void)
{
	...
	UpdateState(
		nullptr, (pev->spawnflags & SF_WATCHERRATIO_FIRSTUPDATE) && (pev->spawnflags & SF_WATCHERRATIO_FIREONSTART));
}
...
void CWatcherRatio::UpdateState(CBaseEntity* pLocus, bool mustTrigger)
{
	...
	if (debugString)
	{
		ALERT(at_console, "watcher_number \"%s\" %s (%f %s %f with tolerance %f)\n", STRING(pev->targetname),
		      debugString, testVal, opString, cmpVal, toleranceVal);
	}
...
void CRenderFxManager::Affect(CBaseEntity* pTarget, BOOL bIsFirst, CBaseEntity* pActivator)
{
	...
	else
	{
		...
		if (pev->noise)
			pFader->m_fOffsetScale = CalcLocus_Number(pActivator,STRING(pev->noise)) - pevTarget->scale;
			//AJH Allows LR scale
		else
...
void CTriggerHurt::HurtTouch(CBaseEntity* pOther)
{
	...
	if (fldmg < 0)
		...
		pOther->TakeDamage(pev, (m_hActivator) ? m_hActivator->pev : pev, fldmg, m_bitsDamageInflict);
	//AJH give frags to activator
...
BOOL CTriggerOnSight::VisionCheck(void) //AJH modifed to check if multiple entities can see
{
	...
	if (pev->netname)
	{
		...
		{
			if (!(pev->spawnflags & SF_ONSIGHT_STATECHECK) || (pev->spawnflags & SF_ONSIGHT_STATECHECK && pLooker->
				GetState() != STATE_OFF))
			{
				CBaseEntity* pSeen;
				if (pev->message)
...
void CTriggerMotion::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	...
	if (m_iszAngles)
	{
		...
		case 1:
			vecTemp = CalcLocus_Velocity(this, pActivator, STRING(m_iszAngles));
			//LRC 1.8 - was using m_iszVelocity by mistake
			if (pev->spawnflags & SF_MOTION_DEBUG)
	...
	if (m_iszAVelocity) //AJH (you forgot to add this Laurie!!)
	{
		...
		case 0:
			...
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_debug, "DEBUG: Set avelocity from %f %f %f ", pTarget->pev->avelocity.x,
...
void CMotionThread::Think(void)
{
	...
	if (pev->spawnflags & SF_MTHREAD_STEP)
	{
	...
	if (m_iszPosition)
	{
		...
		case 1: // offset position (= fake velocity)
			...
			{
				//AJH Affect this axis behaviour (NEW SOHL 1.4)
				Vector tmp = m_hTarget->pev->origin + gpGlobals->frametime * CalcLocus_Velocity(
					this, m_hLocus, STRING(m_iszPosition));
				Vector tmp2 = m_hTarget->pev->origin;
				if (m_iPosAxis & MT_AFFECT_X)
					tmp2.x = tmp.x;
		...
		case 3: // accelerate
			...
			{
				//AJH Affect this axis behaviour (NEW SOHL 1.4)
				Vector tmp = m_hTarget->pev->velocity + gpGlobals->frametime * CalcLocus_Velocity(
					this, m_hLocus, STRING(m_iszPosition));
				Vector tmp2 = m_hTarget->pev->velocity;
				if (m_iPosAxis & MT_AFFECT_X)
					tmp2.x = tmp.x;
		...
		case 0: // set angles
			...
			if (vecTemp != g_vecZero) // if the vector is 0 0 0, don't use it
			{
				...
				else
				{
					...
					UTIL_AssignAngles(m_hTarget, tmp2);
				}
		...
		case 1: // offset angles (= fake avelocity)
			...
			if (vecTemp != g_vecZero) // if the vector is 0 0 0, don't use it
			{
				...
				if (!m_iFaceAxis)
				{
					//AJH Default Behaviour (ORIGINAL)
					UTIL_AssignAngles(
						m_hTarget, m_hTarget->pev->angles + gpGlobals->frametime * UTIL_VecToAngles(vecTemp));
				}
				else
				{
					//AJH Affect this axis behaviour (NEW SOHL 1.4)
...
void CMotionManager::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	...
	{
		if (pThread == nullptr)
			//AJH we shouldn't need this but for some reason the pointer is ALWAYS NULL after save/restore.
		{
			pThread = GetClassPtr(static_cast<CMotionThread*>(nullptr));
