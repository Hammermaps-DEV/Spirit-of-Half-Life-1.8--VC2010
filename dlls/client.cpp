...
void Host_Say(edict_t* pEntity, int teamonly)
{
	...
	client = nullptr;
	while (((client = static_cast<CBasePlayer*>(UTIL_FindEntityByClassname(client, "player"))) != nullptr) && (!
	...
	else if (FStrEq(pcmd, "inventory")) //AJH - Inventory system
	{
		...
		if (CMD_ARGC() > 1)
		{
			if (FStrEq(CMD_ARGV(1), "1"))
			{
				...
				GetClassPtr(static_cast<CItemMedicalKit*>(nullptr))->Use(pPlayer, pPlayer, USE_TOGGLE, 0);
			}
			else if (FStrEq(CMD_ARGV(1), "2"))
			{
				//	ALERT(at_debug,"DEBUG: calling antitox::use()\n");
				GetClassPtr(static_cast<CItemAntidote*>(nullptr))->Use(pPlayer, pPlayer, USE_TOGGLE, 0);
			}
			else if (FStrEq(CMD_ARGV(1), "3"))
			{
				//	ALERT(at_debug,"DEBUG: calling antirad::use()\n");
				GetClassPtr(static_cast<CItemAntiRad*>(nullptr))->Use(pPlayer, pPlayer, USE_TOGGLE, 0);
			}
			else if (FStrEq(CMD_ARGV(1), "6"))
			{
				//	ALERT(at_debug,"DEBUG: calling flare::use()\n");
				GetClassPtr(static_cast<CItemFlare*>(nullptr))->Use(pPlayer, pPlayer, USE_TOGGLE, 0);
			}
		...
		else
		{
			CLIENT_PRINTF(pEntity, print_console,
			              UTIL_VarArgs("\"fov\" is \"%d\"\n",
			                           static_cast<int>(GetClassPtr((CBasePlayer*)pev)->m_iFOV)));
		}
	}
...
void ClientUserInfoChanged(edict_t* pEntity, char* infobuffer)
{
	...
	if (pEntity->v.netname && STRING(pEntity->v.netname)[0] != nullptr && !FStrEq(
...
{
	entvars_t* pev = &pEntity->v;
	CBaseSpectator* pPlayer = static_cast<CBaseSpectator*>(GET_PRIVATE(pEntity));
...
void SetupVisibility(edict_t* pViewEntity, edict_t* pClient, unsigned char** pvs, unsigned char** pas)
{
	...
	// for trigger_viewset
	CBasePlayer* pPlayer = static_cast<CBasePlayer*>(CBaseEntity::Instance(static_cast<struct edict_s*>(pClient)));
                  ...
                  unsigned char* pSet)
{
	...
	if (ent != host)
	{
		if (!ENGINE_CHECK_VISIBILITY(static_cast<const struct edict_s*>(ent), pSet))
		{
	...
	state->animtime = static_cast<int>(1000.0 * ent->v.animtime) / 1000.0;
			...
			while (pPlayerItem)
			{
				...
				if (gun && gun->UseDecrement())
				{
					...
					if (II.iId >= 0 && II.iId < 32)
					{
						...
						item->m_flTimeWeaponIdle = Vmax(gun->m_flTimeWeaponIdle, -0.001);
						item->m_flNextPrimaryAttack = Vmax(gun->m_flNextPrimaryAttack, -0.001);
						item->m_flNextSecondaryAttack = Vmax(gun->m_flNextSecondaryAttack, -0.001);
						item->m_fInReload = gun->m_fInReload;
						item->m_fInSpecialReload = gun->m_fInSpecialReload;
						item->fuser1 = Vmax(gun->pev->fuser1, -0.001);
						...
						//						item->m_flPumpTime				= Vmax( gun->m_flPumpTime, -0.001 );
					}
				}
...
void UpdateClientData(const struct edict_s* ent, int sendweapons, struct clientdata_s* cd)
{
	...
	if (sendweapons)
	{
		...
		if (pl)
		{
			...
			{
				CBasePlayerWeapon* gun;
				gun = static_cast<CBasePlayerWeapon*>(pl->m_pActiveItem->GetWeaponPtr());
				if (gun && gun->UseDecrement())
				{
					ItemInfo II;
					memset(&II, 0, sizeof(II));
					...
					{
						cd->vuser2.y = static_cast<CRpg*>(pl->m_pActiveItem)->m_fSpotActive;
						cd->vuser2.z = static_cast<CRpg*>(pl->m_pActiveItem)->m_cActiveRockets;
					}
				}
			}
		}
