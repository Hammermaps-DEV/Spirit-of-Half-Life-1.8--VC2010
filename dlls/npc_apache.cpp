...
void CApache::Flight(void)
{
	...
	// sideways drag
	pev->velocity.x = pev->velocity.x * (1.0 - Vfabs(gpGlobals->v_right.x) * 0.05);
	pev->velocity.y = pev->velocity.y * (1.0 - Vfabs(gpGlobals->v_right.y) * 0.05);
	pev->velocity.z = pev->velocity.z * (1.0 - Vfabs(gpGlobals->v_right.z) * 0.05);
...
BOOL CApache::FireGun()
{
	...
	if (angles.x > m_angGun.x)
		m_angGun.x = Vmin(angles.x, m_angGun.x + 12);
	if (angles.x < m_angGun.x)
		m_angGun.x = Vmax(angles.x, m_angGun.x - 12);
	if (angles.y > m_angGun.y)
		m_angGun.y = Vmin(angles.y, m_angGun.y + 12);
	if (angles.y < m_angGun.y)
		m_angGun.y = Vmax(angles.y, m_angGun.y - 12);
