...
{
	if (pev->noise) pev->scale = CalcLocus_Number(this, STRING(pev->noise), 0);
...
void CEnvMirror::Spawn(void)
{
	...
	if (pev->spawnflags & SF_MIRROR_DRAWPLAYER) CBaseEntity::Create("player_marker", VecBModelOrigin(pev), pev->angles,
	                                                                NULL);
