...
bool TryParseVectorComponentwise(CBaseEntity* pLocus, const char* szText, Vector* OUTresult,
                                 Vector* swizzleBasis = nullptr, bool isPYR = false)
{
...
bool TryParseLocusBrackets(CBaseEntity* pLocus, const char* szText, char* OUTszPreBracket, char* OUTszPostBracket,
                           Vector* swizzleBasis = nullptr, bool isPYR = false)
{
	int numBrackets = 0;
	int bracketStartIdx;
...
bool TryParseLocusNumber(const char* szText, float* OUTnumber, Vector* swizzleBasis, bool isPYR)
{
	...
	if (swizzleBasis)
	{
		...
		else if (!isPYR)
		{
			...
			if (FStrEq(szText, "YAW"))
			{
				Vector ang = UTIL_VecToAngles(*swizzleBasis);
				*OUTnumber = factor * ang.y;
				return true;
			}
			if (FStrEq(szText, "LENGTH"))
			{
...
CBaseEntity* CalcLocusParameter(CBaseEntity* pLocus, const char* szParamName, Vector* swizzleBasis, bool isPYR)
{
	...
	if (TryParseLocusNumber(szParamName, &tryNumberResult, swizzleBasis, isPYR))
	{
...
bool TryCalcLocus_Velocity(CBaseEntity* pEntity, CBaseEntity* pLocus, const char* szText, Vector* OUTresult,
                           Vector* swizzleBasis)
{
	// blank = 0 0 0
	if (szText[0] == 0)
...
bool TryCalcLocus_PYR(CBaseEntity* pEntity, CBaseEntity* pLocus, const char* szText, Vector* OUTresult,
                      Vector* swizzleBasis)
{
...
bool TryCalcLocus_NumberNonRandom(CBaseEntity* pLocus, const char* szText, float* OUTresult, Vector* swizzleBasis,
                                  bool isPYR)
{
...
bool CCalcPosition::CalcPosition(CBaseEntity* pLocus, Vector* OUTresult)
{
	...
	if (pev->spawnflags & SF_CALCPOSITION_DEBUG)
		ALERT(at_debug, "%s \"%s\" returns {%f %f %f}\n", STRING(pev->classname), STRING(pev->targetname), OUTresult->x,
		      OUTresult->y, OUTresult->z);
	return true;
}
...
bool CCalcNumFromEnt::CalcNumber(CBaseEntity* pLocus, float* OUTresult)
{
	...
	if (target)
	{
		...
		if (pev->impulse == 1)
		{
			...
			if (target->IsPlayer())
			{
				*OUTresult = static_cast<CBasePlayer*>(target)->HasWeapons() ? 1 : 0;
				return true;
			}
		}
...
inline bool TryCalcLocus_VelocityNoSwizzle(CBaseEntity* pEntity, CBaseEntity* pLocus, const char* szText,
                                           Vector* OUTresult)
{
	return TryCalcLocus_Velocity(pEntity, pLocus, szText, OUTresult);
}

inline bool TryCalcLocus_NumberNoSwizzle(CBaseEntity* pEntity, CBaseEntity* pLocus, const char* szText,
                                         float* OUTresult) { return TryCalcLocus_Number(pLocus, szText, OUTresult); }
...
public:
	...
	bool CalcWithFallback(CBaseEntity* pLocus, R* OUTresult,
	                      bool (*CalcFunction)(CBaseEntity* pEntity, CBaseEntity* pLocus, const char* szText,
	                                           R* OUTresult))
	{
...
bool CCalcVelocityPath::CalcVelocity(CBaseEntity* pLocus, Vector* OUTresult)
{
	...
	if (pev->spawnflags & SF_CALCVELPATH_DEBUG)
		ALERT(at_debug, "%s \"%s\" traces from {%f %f %f} to {%f %f %f}, result {%f %f %f}\n", STRING(pev->classname),
		      STRING(pev->targetname), vecStart.x, vecStart.y, vecStart.z, vecEnd.x, vecEnd.y, vecEnd.z, vecOffs.x,
		      vecOffs.y, vecOffs.z);
	*OUTresult = vecOffs;
	return true;
}
		...
		if (!TryCalcLocus_Velocity(this, pLocus, STRING(pev->noise2), &vecResult, &vecResult))
		{
			if (pev->spawnflags & SF_CALCVELPOLAR_DEBUG)
				ALERT(at_debug, "%s \"%s\" failed, bad swizzle \"%s\"\n", STRING(pev->classname),
				      STRING(pev->targetname), STRING(pev->noise2));
			return false;
		}
	}
	...
	*OUTresult = vecResult;
	...
	if (pev->spawnflags & SF_CALCVELPOLAR_DEBUG)
		ALERT(at_debug, "%s \"%s\": basis %f %f %f, returns %f %f %f\n", STRING(pev->classname),
		      STRING(pev->targetname), vecBasis.x, vecBasis.y, vecBasis.z, OUTresult->x, OUTresult->y, OUTresult->z);
