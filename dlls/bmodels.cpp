...
void CFuncWall::Spawn()
{
	...
	UTIL_StringToVector(static_cast<float*>(pev->angles), STRING(pev->message));

	if (pev->angles != g_vecZero)
		ALERT(at_debug, "Rotating brush %s to %i,%i,%i\n",STRING(pev->model), pev->angles.x, pev->angles.y,
		      pev->angles.z);
...
void CFuncConveyor::UpdateSpeed(float speed)
{
	...
	int speedCode = static_cast<int>((Vfabs(speed) * 16.0));
...
void CPendulum::Spawn()
{
	...
	m_accel = (pev->speed * pev->speed) / (2 * Vfabs(m_distance));
