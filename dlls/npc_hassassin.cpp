...
void CHAssassin::Shoot()
{
	...
	{
	case 0: EMIT_SOUND(ENT(pev), CHAN_WEAPON, "weapons/pl_gun1.wav", RANDOM_FLOAT(0.6, 0.8), ATTN_NORM);
		break;
	case 1: EMIT_SOUND(ENT(pev), CHAN_WEAPON, "weapons/pl_gun2.wav", RANDOM_FLOAT(0.6, 0.8), ATTN_NORM);
		break;
	}
...
void CHAssassin::RunAI()
{
	...
	if (pev->renderamt > m_iTargetRanderamt)
	{
		...
		pev->renderamt = Vmax(pev->renderamt - 50, m_iTargetRanderamt);
		pev->rendermode = kRenderTransTexture;
	}
	else if (pev->renderamt < m_iTargetRanderamt)
	{
		pev->renderamt = Vmin(pev->renderamt + 50, m_iTargetRanderamt);
		if (pev->renderamt == 255)
